    ...
    <div class="py-4">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Publish fresh content to your blog.</p>
            </div>
        ...
        <form method="POST" action="{{ route('cms.blog.posts.store') }}" class="mt-4 space-y-4" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Max 4MB</p>
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Short Excerpt</label>
                <textarea name="excerpt" rows="2" class="mt-1 w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Optional summary shown in lists">{{ old('excerpt') }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Content</label>
                <textarea name="content" rows="10" class="mt-1 w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>{{ old('content') }}</textarea>
            </div>
